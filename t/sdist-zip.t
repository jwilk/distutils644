...
rm -rf dist
{ $python setup.py sdist --formats=zip > /dev/null; } 2>&1
...
if zipinfo --h-t dist/distutils644-*.zip | { ! grep -v -E '^(drwxr-xr-x|-rw-r--r--|-rwxr-xr-x) '; }
then
    echo ok 1 644/755 permissions
else
    echo not ok 1 unexpected permissions
fi
if zipinfo -v dist/distutils644-*.zip | { ! grep -w 'Unix UID'; }
then
    echo ok 2 no UNIX UIDs
else
    echo not ok 2 unexpected UNIX UIDs
fi
if zipinfo -1 dist/distutils644-*.zip | grep -E '^[^/]+/[^/]+$' | LC_ALL=C sort -c
then
